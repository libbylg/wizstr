...
TEST(test_str, prev_eol_view) {
    SECTION("一般情况:LF") {
        std::string_view s{"a\nb\nc"};
        str::size_type pos = str::npos;
        std::optional<std::string_view> result;

        ASSERT_TRUE(result = str::prev_eol_view(s, pos));
        ASSERT_TRUE((pos == 3) && (*result == "\n"));
        ASSERT_TRUE(result = str::prev_eol_view(s, pos));
        ASSERT_TRUE((pos == 1) && (*result == "\n"));
        ASSERT_FALSE(result = str::prev_eol_view(s, pos));
        ASSERT_TRUE((pos == 0));
        ASSERT_FALSE(result = str::prev_eol_view(s, pos));
        ASSERT_TRUE((pos == 0));
    }
    ...
    SECTION("pos超出字符串范围[1]") {
        ...
        ASSERT_TRUE(result = str::prev_eol_view(s, pos));
        ASSERT_TRUE((pos == 3) && (*result == "\n"));
        ASSERT_FALSE(result = str::prev_eol_view(s, pos = 1));
        ASSERT_TRUE((pos == 0));
    }
