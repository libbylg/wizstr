...
TEST(test_str, join_list) {
    SECTION("一般场景") {
        ...
        ASSERT_EQ(str::join_list("|", std::vector{"A", "B", "C"}), "A|B|C");
        ASSERT_EQ(str::join_list("", std::vector{"A", "B", "C"}), "ABC");
        ASSERT_EQ(str::join_list("|", std::vector<str::const_pointer>{}), "");
        ASSERT_EQ(str::join_list("|", std::vector{"A", "", "C"}), "A||C");
        ASSERT_EQ(str::join_list("/", std::list{"A", "B", "C"}), "A/B/C");
        ASSERT_EQ(str::join_list("*", std::array{"A", "B", "C"}), "A*B*C");
