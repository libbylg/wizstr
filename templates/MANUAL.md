...
* 字符串拼接：@{#join}
* 大小写转换：@ref{#to_upper}, @ref{#to_lower}, @ref{#to_capitalize}, @ref{#swap_case}
...
对于 `append_inplace` 函数，如果 s 与 被插入字符串存在重叠时，函数的行为是不确定的，应该避免出现这种
情况。



### 向头部追加 - prepend @anchor{prepend}
...
### 不区分大小写的比较 - icompare @anchor{icompare}
...
### 不区分大小写的相等測試 - iequals @anchor{iequals}
...
### 基于通配符的匹配检测 - wildcmp, iwildcmp @anchor{wildcmp, iwildcmp}
...
### 判断两个字符串的包含关系 - contains @anchor{contains}
...
@param s: 目标字符串
@param other: 参与公共前缀计算的另一个字符串，用于 `prefix`
@param prefix, ch: 前缀字符串或者字符



### 后缀操作 - suffix @anchor{suffix}
...
@param s: 目标字符串。
@param other: 参与公共前缀计算的另一个字符串，用于 `suffix`。
@param suffix, ch: 后缀字符串或者字符。



### 检测是否以特定的模式开头和结束 - spaces, margin @anchor{spaces_margin}
...
### 定位字符位置 - next_char, prev_char @anchor{next_char, prev_char}
...
需要特别注意，next_xxx 总是从 `pos - 1` 开始，向字符串首部查找（而 `next_char` 总是从
...
当找到特定的字符时，`next_char` 的 `pos` 总是为该字符的下一个字符，而 `pref_char` 的
`pos` 总是指向该字符所在的位置。这个设计使得 `next_char` 和 `prev_char` 可以配合使用。

% 参数

@param s: 在该字符串中查找
@param pos: 查找的起始位置，需要注意该字段对于 `next_xxx` 和 `prev_xxx` 具有不同的含义。
@param ch, charset: 用于定位的字符或者字符集。
@param proc: 用于测试某个字符是否满足定位条件，常用于定制场景。



### 定位子串 - next_string, prev_string @anchor{next_string, prev_string}
...
当找到特定的字符时，next_xxx 的 `pos` 总是为该字符的下一个字符，而 pref_xxx 的 `pos`
...
### 定位行结束符（换行符）- next_eol, prev_eol @anchor{next_eol, prev_eol}
...
### 行结束符检测 - ends_with_eol @anchor{eol, ends_with_eol, eol_suffix}
...
### 移除行结束符 - remove_eol @anchor{remove_eol}
...
### 定位空白块 - next_spaces @anchor{next_spaces}
...
“空白块”是指由 @ref{#all_spaces} 中的字符组成的连续的子串。
...
### 特征测试：常见词法特征类 - is_literal, is_identifier @anchor{is_literal, is_identifier}
...
### 特征测试：指定字符集类 - is_all_in @anchor{is_all_in}
...
### 特征测试：单一条件类 - has_any_one @anchor{has_any_one}
...
### 多行文本处理 - lines @anchor{lines, foreach_lines, count_lines}
...
### 以单词为单位的处理算法 - words @anchor{words}
...
### 用指定的模式串环绕字符串 - surround, unsurround @anchor{surround, unsurround}
...
`spaces`，`make_spaces`，`make_spaces_inplace` 这三个函数的功能类似。
...
当 `left_n` 或者 `right_n` 或者 `left_n + right_n` 大于或者等于原始串 s 的长度时，本函数将失去
...
### 拼接列表 - join_list @anchor{join_list}
...
### 按行拼接 - join_lines @anchor{join_lines}
...
### 拼接路径 - join_path @anchor{join_path}
...
### 拼接搜索路径 - join_searchpath @anchor{join_searchpath}
...
### 按逗号拆分 - split_list @anchor{split_list}
...
### 拆分 key-value 对 - split_pair @anchor{split_pair}

```c++
@embed split_pair: include/str.hpp
```

* split_pair 等价于 @ref{#split} 系列函数，指定 `max_n` 参数为 `1` 时的功能。
* <<
...
### 拆分多个 key-value 对 - split_map @anchor{split_map}
...
### 按行拆分 - split_lines @anchor{split_lines}
...
### 路径拆分 - split_path @anchor{split_path}
...
本函数并不支持 windows 下同时可以使用 `'/'` 和 `'\\'` 为分隔符的场景。如果有必要，可以使用
...
### 拆分搜索路径 - split_searchpath @anchor{split_searchpath}
...
需要注意 POSIX 系统标准并不允许文件路径中存在冒号的情况，
参考: https://pubs.opengroup.org/onlinepubs/9699919799/basedefs/V1_chap08.html#tag_08_03。

需要注意本函数默认支持的是 `*nix` 系统的默认的搜索路径分隔符，对于 Windows 系统，可以
指定 `sep` 参数为 `";"`。

本系列函数并不会主动去除路径前后的空白，也对于重复路径也不去重。当 `keep_empty` 指定
为 `true` 时，空串会被传递给 proc 或者返回。

% 参数

@param s: 待拆分的路径
@param keep_empty: 是否保留空路径（注意POSIX 中空搜索路径通常表示当前工作路径，是有意义的）
...
### 字符串分块 - next_chunk @anchor{next_chunk}
...
### 指定宽度拆分字符串 - chunked @anchor{chunked}
...
### 字符串分块 - next_window @anchor{next_window}
...
### 基于窗口拆分字符串 - windowed @anchor{windowed}
...
### 大小写转换 - to_upper, to_lower, to_capitalize, swap_case  @anchor{to_upper, to_lower, to_capitalize, swap_case}
...
### 化简 - simplified @anchor{simplified}
...
### 展开 - expand_envs @anchor{expand_envs}
...
### tab 扩展 - expand_tabs @anchor{expand_tabs}
...
### HOME 字符扩展 - expand_user @anchor{expand_user}
...
### 路径正规化 - normpath @anchor{normpath}
...
需要注意，本算法（函数）只是做字符串的操作，并不会访问文件系统。但这也意味着，在部分场景下
并不能完全去掉相对路径。
...
### 路径的基本部分的长度 - basename_pos, extname_pos, dirname_pos @anchor{basename_pos, extname_pos, dirname_pos}
...
### 路径处理函数：目录 - dirname @anchor{dirname}
...
### 路径处理函数：文件名 - basename @anchor{basename}
...
### 路径处理函数：扩展名 - extname @anchor{extname}
...
### 路径处理函数：裸文件名 - rawname @anchor{rawname}
...
### 转义：C语言字符串 - encode_cstr, decode_cstr @anchor{encode_cstr, decode_cstr}
...
### 编解码：base64 - encode_base64, decode_base64 @anchor{encode_base64, decode_base64}
...
### 编解码：base16 - encode_base16, decode_base16 @anchor{encode_base16, decode_base16}
...
### 数据按十六进制 - dump_hex @anchor{dump_hex}
...
% 格式控制 - dump_hex_format @anchor{dump_hex_format}

下面为数据格式控制的数据结构，最终会作为 `dump_hex` 的第三个参数。

```c++
@embed dump_hex_format: include/str.hpp
```

下面是 `dump_hex` 格式化数据的一个示例。可以看出`dump_hex` 函数输出的格式完整形式可分为三个区域。
这个的示例是以冒号 `:` 分隔了每个区域：
...
### 字符集对象生成 - charset @anchor{charset}
...
### 范围对象生成 - range, interval, shifter @anchor{range, interval, shifter, range_interval_shifter}
...
`shifter` 函数相对比较特殊，其 `offset` 参数可以为正值也可以为负值。如果 `offset` 为负值，
可以理解为 `(pos + offset)` 到 `pos` 的范围（不包含 `pos`）。如果 `offset` 为正值，可以理解为 `pos`
到 `(pos + offset)` 的范围。当然在遇到具体字符串时，会根据具体字符串的长度调整。

% 类型

* range_type @anchor{range_type}

使用 `pos` 和 `len` 表示一个字符串片段的范围：

```c++
    struct range_type {
@embed range_type: include/str.hpp
    };
```

* shifter_type @anchor{shifter_type}

使用 `pos` 和 `offset` 来表示一个字符串片段的范围，需要注意 `offset` 为正或者负时，含义不同。

```c++
@embed shifter_type: include/str.hpp
```

* interval_type @anchor{interval_type}

使用 `begin` 和 `end` 表示的一个字符串片段的范围。需要注意，`begin` 必须不大于 `end`。

```c++
@embed interval_type: include/str.hpp
```

* <<


### 读取一行 - read_next_line @anchor{read_next_line}
...
### 读取文件的全部内容 - read_all @anchor{read_all}
...
### 按行读取多行 - read_lines @anchor{read_lines}
...
本系列函数和 @ref{#read_all} 函数，只适合文件内容较少的文本文件的读取，并不适合大型文件。
...
### 打开文件并自动关闭文件 - with_file @anchor{with_file}
...
当 `proc` 函数使用完毕后，`with_file` 函数会自动关闭该文件。本函数设计的相对特殊，其核心目的是
...
@param proc: 用于接收并使用被 `with_file` 打开的文件对象的函数。



### 命令行选项识别 - next_opt1 @anchor{next_opt1}

```c++
@embed next_opt1: include/str.hpp
```

将一组字符串列表视作命令行的参数（选项）序列，`next_opt1` 函数从该序列中按照特定的模式读取和识别
出单个的命令行选项。

% 注意

与很多其他的命令行参数识别库不同，`next_opt1` 函数所支持的命令行参数的识别模式是一
种无 schema 的模式，其优点是不需要在参数识别前定义一堆的 schema 信息（比如，需要定义有哪些命令行参
数，每个参数的类型），所以使用起来相对方便快捷。但，相应的，`next_opt1` 无法实现很复杂的命令
行参数设计。`next_opt1` 很适合具有少量命令行参数的情况。下面是 `next_opt1` 函数的识别算
法：
...
### 命令行选项识别 - next_opt2 @anchor{next_opt2}

```c++
@embed next_opt2: include/str.hpp
```

将一组字符串列表视作命令行的参数（选项）序列，`next_opt2` 函数从该序列中按照特定的模式读取和识别
出单个的命令行选项。

% 注意

与很多其他的命令行参数识别库不同，@{#next_opt2} 函数所支持的命令行参数的识别模式是一
种无 schema 的模式，其优点是不需要在参数识别前定义一堆的 schema 信息（比如，需要定义有哪些命令行参
...
accept 和 accept_until 这两组函数的行为的重要区别是：accept 系列函数总是 '盯着' 满足
条件的字符，而 accept_until 总是 '盯着' 不满足条件的字符。

虽然 accept 和 accept_until 这两组函数都是可以连续调用的，但是需要注意 accept_until 总是
...
### 符号识别 - skip_spaces @anchor{skip_spaces}
...
### 用户根目录 - home @anchor{home}

```c++
@embed home: include/str.hpp
```
...
由于 Windows 下并无严格意义上的与 `*nix` 下对等的用户根目录的概念，因此，
