...
TEST(test_str, trim_anywhere_inplace) {
    ...
    SECTION("左边有空白") {
        ASSERT_EQ(str::trim_anywhere_inplace(s = "\r  \t\nc1233"), "c1233");
    }

    SECTION("右边有空白") {
        ASSERT_EQ(str::trim_anywhere_inplace(s = "c1233\r  \t\n"), "c1233");
    }
    ...
    SECTION("指定字符") {
        ...
        // 只去掉前三个匹配的字符
        size_t n = 0;
        ASSERT_TRUE(str::trim_anywhere_inplace(s = "33c1233", [&n](std::string::value_type ch) -> bool {
            if (n >= 3) {
                return false;
            }
